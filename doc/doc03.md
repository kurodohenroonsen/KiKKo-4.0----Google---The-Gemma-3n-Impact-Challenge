...
*   **Proactive Quests (Optional):** The Hive can be proactive (if enabled by the user). Using on-device sensors and existing knowledge, it can generate quests.
    *   *Example for Hiro:* "We've detected a unique insect, Forager! The 'pollen' of this creature is unknown. Shall we capture its details?"
...
*   **Initial Extraction (Worker Bees - ML Kit):** As pollen enters the Hive, the Worker Bees immediately begin processing it in parallel.
...
*   **Filling the Honeycomb & Microsite Creation:** The empty quest honeycomb animates, filling with shimmering, golden liquid honey. This "Informative Honey" is then presented as a browsable, multi-language **"Microsite"** (e.g., dedicated to the ladybug forage).
