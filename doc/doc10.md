...
*   **The AI Queen (Gemma - The Partner):** At the heart of the Hive is **Gemma**, leveraged for its powerful on-device reasoning. The Queen acts not as an oracle, but as a collaborative partner:
    1.  **Suggestion Engine:** It suggests `schema.org` classifications (e.g., `schema:Ladybug`, `schema:Plant`) and maps extracted data to properties.
    2.  **Dialogue Generation & Contextual Querying:** It formulates natural language questions for the Bourdon to relay to the user for clarification and to gather crucial human context (e.g., "Where exactly did you find this ladybug? What plant was it on?"). This human-in-the-loop approach is fundamental to building truly personal and verifiable knowledge.
    3.  **Complex Structuring & Microsite Generation:** It structures nested data types (e.g., `PostalAddress`, `ContactPoint`) and understands relationships between different pieces of pollen. Once structured and validated, it generates the user-friendly, multi-language **"Microsite"** for the "Informative Honey."
    ...
    5.  **Inference Reproduction Blueprint:** It logs its entire reasoning chain, including all prompts, intermediate steps, and user inputs, within the "Thread of Provenance," creating a verifiable blueprint for other Hives to reproduce the inference locally.
...
*   **The Worker Bees (ML Kit - The Specialists):** A comprehensive suite of **ML Kit** APIs acts as our specialized "Worker Bees" for fast, efficient, on-device data extraction from the real world. Their diverse capabilities are crucial for transforming raw pollen into structured inputs for Gemma:
    *   **Vision:**
        ...
        *   **Image Labeling (Base & Custom Models):** Identifies objects and concepts within images (e.g., "ladybug," "leaf," "flower").
        ...
        *   *(Used for context in provenance):* Face Detection/Mesh (identifying Hiro's face in selfies), Pose Detection (Hiro's gestures), Selfie Segmentation, Subject Segmentation (isolating the ladybug from its background for precise analysis).
    *   **Natural Language:**
        *   **Language Identification:** Determines language of text/speech.
        *   **Translation:** Used for internal schema localization and for translating the final "Microsite" content into multiple languages for user viewing and sharing.
...
*   **The Bourdon (The Communicator & The Hornet's Messenger):** This unique AI agent manages the interactive dialogue and presents the core dilemma.
    1.  **Dialogue Orchestration:** Relays Queen's quests, suggestions, and contextual questions via TTS.
    2.  **Hornet's Offer Delivery with Quiz:** Activates a local **WebView** to perform a Google Search query (e.g., `Coccinelle à sept points; what is it and fun facts, impacts and quiz?`). It then uses **ML Kit's OCR and Entity Extraction** to scrape and parse the "AI Overview" and other structured data (fun facts, quiz questions) from the WebView's HTML. This processed "Hornet's Pollen" is then presented to the user via TTS (voiced by the Bourdon) with an interactive multiple-choice quiz, engaging the user (Hiro in his red shirt) while the Queen works on the pure pollen.
    3.  **Choice Presentation:** Presents the user with the crucial choice: accept the Hornet's quick, opaque answer, or wait for the Queen's verified, pure honey with full provenance and reproducibility.
...
*   **The Architecture of Trust & Digital Pollination:**
    ...
    *   **Decentralized Sharing:** **WebTorrent** is integrated for secure, private, peer-to-peer sharing of "Trusted Packages" (structured Microsite data + provenance + raw pollen). Crucially, the receiving Hive uses its local Gemma and ML Kit to **reproduce the inference** based on the sender's provenance log, verifying the data's integrity before integrating it.

| Introduction | Action | Conclusion |
| :---: | :---: | :---: |
| <img src="illustrations/tech_intro_v2.png" alt="Cinematic 3D render, animation movie style. A stream of raw 'pollen' data (e.g., a photo of a ladybug on a leaf) flows into the Hive's main processing chamber. Small, diligent Worker Bees (ML Kit robots) immediately start extracting data with light beams, but their progress bar is moving slowly."> | <img src="illustrations/tech_action_v2.png" alt="Cinematic 3D render, animation movie style. The plump Bourdon, with a smug grin, floats in front of the Worker Bees, holding a glowing digital 'AI Overview' from a web search, complete with a quiz interface. He gestures towards Hiro (with red shirt), offering the instant answer as a tempting shortcut. Meanwhile, the Queen watches calmly, awaiting Hiro's choice and preparing contextual questions."> | <img src="illustrations/tech_conclusion_v2.png" alt="Cinematic 3D render, animation movie style. If Hiro chooses purity, the Bourdon's 'AI Overview' vanishes. The AI Queen (Gemma) confidently weaves the pure raw pollen from the Worker Bees into a shimmering thread of perfect knowledge, incorporating Hiro's contextual answers. A dazzling 'Seal of Trust' forms, indicating the data is ready for inference reproduction and multi-language Microsite generation."> |
| **1. Initial Extraction:** As raw pollen (user input, e.g., a ladybug photo) enters the Hive, Worker Bees (ML Kit) begin slow, meticulous on-device processing. | **2. The Bourdon's Offer:** Simultaneously, the Bourdon delivers an instant, pre-packaged "AI Overview" (from a local WebView search) complete with a quiz, as a tempting shortcut, posing a dilemma to Hiro (with red shirt). | **3. The Forger's Path:** If the user chooses authenticity, the Bourdon's 'AI Overview' vanishes. The AI Queen (Gemma) takes the pure pollen and forges verifiable knowledge, culminating in a pristine "Seal of Trust" and a multi-language Microsite, all capable of inference reproduction. |
...
*   The **magic** of Kikko's unique UI as a "window" into a personal Hive, showing Hiro (in his red shirt) foraging a ladybug.
*   The **temptation** of the Hornet's instant, opaque answers, delivered by the charmingly lazy Bourdon (and his engaging quiz).
*   The **satisfaction** and profound value of forging one's own, verifiable "honey" with the AI Queen, including the collaborative contextual questions.
*   The **reassurance** of the "Thread of Provenance" and the revolutionary concept of **Inference Reproduction** in the face of digital chaos.
*   The **long-term utility** of an AI companion that truly understands *your* world and builds multi-language "Microsites."
